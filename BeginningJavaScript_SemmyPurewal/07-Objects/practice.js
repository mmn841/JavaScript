...
var isUser = function (user) {
    ...
    if(typeof(user) === "object") {
        ...
        if(keys.length === 2) {
            if(typeof(user.name) === "string" && typeof(user.screen_name) === "string") {
                result = true;
            }
        }
    }
...
var userToDiv = function (user) {
    ...
    result = "<div><h1>" + user.name + "</h1><h2>" + user.screen_name + "</h2></div>";
    return result;
};
...
//
var frequencies = function (lst) {
    if(!(Array.isArray(lst))) {
        throw "Invalid input";
    }

    var result = {};
    lst.reduce(function(obj, word){
        var count;

        if(obj !== null && obj !== undefined) {
            if(Object.keys(obj).indexOf(word) !== -1) {
                count = obj[word];
                count++;
                obj[word] = count;
            } else {
